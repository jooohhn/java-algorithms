 ...
 * characters.
 *
 * Solution: Use a "sliding window" 1) For each character in the string, create a sliding window of
 * duplicate characters 2) If longest is reassigned, then we can assume all charcters between back
 * and front have no duplicates
 *
 * @Time: O(n) - Each character is visited only twice (by front and back)
