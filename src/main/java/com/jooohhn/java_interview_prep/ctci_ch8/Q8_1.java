 ...
 * stairs.
 *
 * Solution: Similar to fibbonaci dynamic programming, have a table that stores already
 * calculated results @Time: O(n) @Space: O(n)
 */
public class Q8_1 {
  ...
  private static int helper(int stairSteps, int[] table) {
    ...
    table[stairSteps] =
        helper(stairSteps - 1, table)
            + helper(stairSteps - 2, table)
